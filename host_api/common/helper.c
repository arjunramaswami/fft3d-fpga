...
void get_sp_input_data(float2 *fft_data, fftwf_complex *fftw_data, unsigned N[3], char *fname){
  ...
  if(fp != NULL){
      printf("SP: Scanning floats from file - %s\n\n",fname);
      for (i = 0; i < N[0]; i++) {
        for (j = 0; j < N[1]; j++) {
          for ( k = 0; k < N[2]; k++) {
            where = coord(N, i, j, k);
      ...
      printf("SP: Data not available. Printing random floats to file - %s\n",fname);
      fp = fopen(fname,"w");
      for (i = 0; i < N[0]; i++) {
        for (j = 0; j < N[1]; j++) {
          for ( k = 0; k < N[2]; k++) {
...
void get_dp_input_data(double2 *fft_data, fftw_complex *fftw_data, unsigned N[3], char* fname){
  ...
  FILE *fp = fopen(fname,"r");
  if(fp != NULL){
    printf("DP: Scanning doubles from file - %s\n\n",fname);
    ...
    printf("DP : Data not available. Printing random doubles to file - %s\n",fname);
...
double compute_sp_fftw(fftwf_complex *fftw_data, int N[3], int inverse){
  ...
  printf("SP: Planning %sSingle precision FFTW ... \n", inverse ? "inverse ":"");
  if(inverse){
  ...
  printf("SP: Computing Single Precision FFTW\n");
...
double compute_dp_fftw(fftw_complex *fftw_data, int N[3], int inverse){
  ...
  printf("DP: Planning %sDouble precision FFTW ... \n", inverse ? "inverse ":"");
  if(inverse){
  ...
  printf("DP: Computing Double Precision FFTW\n");
  double start = getTimeinMilliSec();
  fftw_execute(plan);
  double stop = getTimeinMilliSec();
...
void compute_metrics( double fpga_runtime, double fftw_runtime, unsigned iter, int N[3]){
  ...
  if(fftw_runtime != 0.0){
    ...
    double gpoints_per_sec = ( N[0] * N[1] * N[2] / (fftw_runtime * 1E-6)) * 1E-9;
    double gflops = 3 * 5 * N[0] * N[1] * N[2]* (log((double)N[0])/log((double)2))/(fftw_runtime * 1E-6 * 1E9);
    printf("\t  %d³ \t\t %.4f \t  %.4f \t\n", N[0], (fftw_runtime * 1E-3), gflops);
    fprintf(fp, "%d,%.4f,%.4f\n", N[0], (fftw_runtime * 1E-3), gflops);
  }
  else{
    printf("ERROR in FFT3d\n");
